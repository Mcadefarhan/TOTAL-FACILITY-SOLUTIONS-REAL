...
export default function SeekerNotifications() {
  ...
  const markAllRead = async () => {
    try {
      await seekerAPI.markNotificationsRead();
      setNotifications((prev) => prev.map((notification) => ({ ...notification, read: true })));
      updateUser({ unreadNotifications: 0 });
      toast.success(t('seekerNotifications.markedAllRead'));
    } catch {
      toast.error(t('seekerNotifications.updateFailed'));
    }
  };
  ...
  return (
    <div className="max-w-2xl animate-fade-in">
      <PageHeader
        title={t('seekerNotifications.title')}
        subtitle={unread > 0 ? formatText(t('seekerNotifications.unreadSubtitle'), { count: unread }) : t('seekerNotifications.allCaughtUp')}
        action={unread > 0 && (
          <button onClick={markAllRead} className="btn btn-ghost btn-sm gap-1.5">
            <CheckCheck className="w-4 h-4" /> {t('seekerNotifications.markAllRead')}
          </button>
        )}
      />
      ...
      {loading ? (
        ...
        <EmptyState
          icon={Bell}
          title={t('seekerNotifications.emptyTitle')}
          description={t('seekerNotifications.emptyDescription')}
        />
      ) : (
        <div className="space-y-3">
          {notifications.map((notification, index) => {
            ...
            return (
              <div key={index} className={clsx('flex gap-4 p-4 rounded-xl border transition-all', style.bg, style.border, !notification.read && 'ring-1 ring-gold-500/20')}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {notification.createdAt && (
                    <p className="text-xs text-gray-400 mt-1.5">
                      {new Date(notification.createdAt).toLocaleDateString(locale, { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}
                    </p>
                  )}
                </div>
              </div>
